...
- **国旗は正解画面に「似ている国旗」＋見分け方**を表示（紛らわしい国旗の取り違え対策）。データは `data/flag_similar.json`（28組）。
- **ボラードは「同じ反射板配置を共有する他国」を補足**（その指標だけでは一意に絞れない国を明示）。
- 解説はすべて日本語（出典の原文は `source_quote` に保持して監査可能）。
...
js/audit.js                  監査ビュー
data/*.json                  データ正本（flags/plates/bollards/roadlines/scripts）
data/coverage.json           ジオゲッサ/SVカバー範囲（出典ベース）
data/flag_similar.json       紛らわしい国旗の組＋見分け方（28組）
data/bundle.js               自動生成（ブラウザが読む）
scripts/build_flags.mjs      国旗データ生成
scripts/build_coverage.mjs   カバー範囲生成
scripts/verify.mjs           検証
scripts/check_quotes.mjs     引用実在監査（5カテゴリ＋flag_similar）
